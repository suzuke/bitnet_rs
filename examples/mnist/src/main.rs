...
struct BitLinearModel {
    linear: BitLinear,
}

impl Model for BitLinearModel {
    fn new(vs: VarBuilder) -> Result<Self> {
        let linear = bitlinear(IMAGE_DIM, LABELS, BitLinearConfig::default(), vs)?;
        Ok(Self { linear })
    }
...
impl Model for BitMlp {
    fn new(vs: VarBuilder) -> Result<Self> {
        let ln1 = bitlinear(IMAGE_DIM, 100, BitLinearConfig { use_before_nonlinear: true, ..Default::default()}, vs.pp("ln1"))?;
        let ln2 = bitlinear(100, LABELS, BitLinearConfig::default(), vs.pp("ln2"))?;
        Ok(Self { ln1, ln2 })
    }
...
impl CnnModel for BitConvNet {
    fn new(vs: VarBuilder) -> Result<Self> {
        let conv1 = candle_nn::conv2d(1, 32, 5, Default::default(), vs.pp("c1"))?;
        let conv2 = candle_nn::conv2d(32, 64, 5, Default::default(), vs.pp("c2"))?;
        let fc1 = bitlinear(1024, 1024, BitLinearConfig {use_before_nonlinear: true, ..Default::default()}, vs.pp("fc1"))?;
        let fc2 = bitlinear(1024, LABELS, BitLinearConfig::default(), vs.pp("fc2"))?;
        let dropout = candle_nn::Dropout::new(0.5);
        Ok(Self {
            conv1,
            conv2,
...
#[derive(Parser, Debug)]
struct Args {
    #[clap(value_enum, default_value_t = WhichModel::BitLinear)]
    model: WhichModel,
...
pub fn main() -> anyhow::Result<()> {
    ...
    match args.model {
        ...
        WhichModel::Cnn => training_loop_cnn::<ConvNet>(m, &training_args),
